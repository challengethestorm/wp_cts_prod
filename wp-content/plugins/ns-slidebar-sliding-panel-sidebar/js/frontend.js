(function($){
  $(function(){
	...
	// set boundaries for sidebar to show - no higher than header, no lower than footer
	var top = $('#ns-slidebar').offset().top - parseFloat($('#ns-slidebar').css('marginTop').replace(/auto/, 0));
    var footTop = $('#ns-slidebar').offset().top - parseFloat($('#ns-slidebar').css('marginBottom').replace(/auto/, 0));

    //var maxY = footTop - $('#ns-slidebar').outerHeight();
    var maxY = $('#footer-main').offset().top - $('#ns-slidebar').outerHeight();

    var headerBottom = $('#header-main').offset().top + $('#header-main').outerHeight(true);
  	var sidebarTop = $('#ns-slidebar').offset().top;
  	var sidebarHeight = $('#ns-slidebar').outerHeight(true);
  	var sidebarBottom = $('#ns-slidebar').offset().top + $('#ns-slidebar').outerHeight(true);
  	var footerTop = $('#footer-main').offset().top;
...
//	$("#ns-slidebar-control").height(500);
	...
	var open_slidebar = function(){
		$('#ns-slidebar').animate( {width:0}, 200, function(){
			$('#ns-slidebar').removeClass('close').addClass('opening').animate( {width:open_width}, 400, function(){
	...
	var close_slidebar = function(){
		$("#ns-slidebar").removeClass('open').addClass('closing').animate( {width:0}, 400, function(){
			$("#ns-slidebar").removeClass('closing').addClass('close').animate( {width:closed_width}, 200 );
		});
	...
	// since people will probably expect it to act as the toggle. Used to be "...-form form" but changed to div
	$('#ns-slidebar-search-form div').click(function(e){
		if( e.target===this ){
			close_slidebar();
		}
	});
