...
//A. Hanna 8/12/2016	//Added retreival of the email address
  	$wp_about_author_author['email'] = get_the_author_meta('email');
...
//A. Hanna 9/3/2016     //added <div> in front of <p class="wpa-nomargin">
                $wp_about_author_content .= '<div><p class="wpa-nomargin">'.apply_filters( 'wp_about_author_follow_me', "Follow Me:").'<br />' . $wp_about_author_social.'</p>';
            }
        } else {
//A. Hanna 9/3/2016     //added <div> in front of <p class="wpa-nomargin">
            $wp_about_author_content .= "<div><p class='wpa-nomargin'>";
...
//A. Hanna 8/12/2016        // About Author Contact
//CONSIDER SWAPPING OUT FOR CONTACTFORM2 C:\wamp64\www\wp_cts_test\wp-content\mu-plugins\contactform\contactform.php would have to also change the email in C:\wamp64\www\wp_cts_test\wp-content\themes\.decorated\plugins-data\instances.php for the author's email???? how would that work?
        if(!empty($wp_about_author_author['email'])){
        	$wp_about_author_email = "<div style='text-align:center; font-style:italic;'><p style='font-size:larger;'><a class='button-blue' style='padding:10px;' href=mailto:".$wp_about_author_author['email'].">Contact the author</a></div>";
//A. Hanna 9/3/2016     //added full line to concat with $wp_about_author_content, wrapping the social follow links and the contact author button within a single div. and added float:right and italics to <p> style 
                $wp_about_author_content .= "<p style='font-size:larger;font-style:italic;float:right;'><a class='button-blue' style='padding:10px;' href=mailto:".$wp_about_author_author['email'].">Contact the author</a></div>";
        }
        else {
        	$wp_about_author_email='';
//A. Hanna 9/3/2016     //added full line to concat with $wp_about_author_content, wrapping the social follow links and the contact author button within a single dif                
                $wp_about_author_content .= "<p style='font-size:larger;'><a class='button-blue' style='padding:10px;' href=mailto:".$wp_about_author_author['email'].">Contact the author</a></div>";
        }
//End author email section
...
function insert_wp_about_author($content) {
    ...
    //set post_type variable to check against
    ...
    if(is_front_page() && isset($wp_about_author_settings['wp_author_display_front']) && $wp_about_author_settings['wp_author_display_front']){
		...
		$content.=wp_about_author_display();
    //added condition that post type has to be POST to consider adding the block
	} else if(is_single() && isset($wp_about_author_settings['wp_author_display_posts']) && $wp_about_author_settings['wp_author_display_posts'] && $post_type == 'post'){
		$content.=wp_about_author_display();
...
function wp_about_author_get_social_links($wp_about_author_settings){
        ...
        foreach($socials as $social_key=>$social){
            if (get_the_author_meta($social_key)){
                if(isset($wp_about_author_settings['wp_author_social_images']) && $wp_about_author_settings['wp_author_social_images']){
                    $content .= "
                    	<a class='wpa-social-icons' href='".str_replace('%%username%%', get_the_author_meta($social_key), $social['link'])."'target='_blank'><img src='".$social['icon']."'alt='".$social['title']."'/></a>";
                } else {
                    if($content != "")
