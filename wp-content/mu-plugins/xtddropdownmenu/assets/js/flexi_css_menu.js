...
(function($) {
	function bindEffect(name, params) {
		...
		function showMenu(liItem, callback) {
			...
			switch(params.effect) {
				...
				case "slide":
					...
					$(self).siblings().find('ul').stop(false, true).removeAttr("style")
					window.setTimeout(function(){
						$(self).find('> ul').show(params.effect, params.options, params.duration, function(){
							if(params.effect == "fold") {
				...
				case "fade":
					uls.stop(true, true);
					$(self).find('> ul').show(params.effect, params.options, params.duration);
					break;
			...
			switch(params.effect) {
				...
				case "slide":
					...
					var timeout = window.setTimeout(function(){
						$(self).find('ul').hide(params.effect, params.options, params.duration, function(){
		...
		$(menu).find("li").mouseenter(function(ev) {
			...
			if ($(self).find('> ul').length) {
				...
				$(self).find('>ul').data('timeout', timeout);
	...
	function stickToTop(name) {
		var menu = $("div.FM_CSS_" + name + "_container");
...
(function($) {
	...
	CSSMenus.mobileParentTemplate = '<div id="FM_CSS_#mobile_name#_mobile_container" class="mobile_menu_button"></div>';

	/*
	 * adds mobile menu container in the page. 
	 * container will be added outside of css menus container
	 */ 
	CSSMenus.insertMobileParent = function (name) 
	{ 
		var containerHtml = CSSMenus.mobileParentTemplate.replace("#mobile_name#", name);
		var cssMenuContainer = $("#" + name + "_container");
		cssMenuContainer.after(containerHtml);
	}
	...
	function xtdCSSMenuInit(Name, options)
	{
		...
		//console.log(viewportWidth + " .. " + options.mobileMaxWidth + " > " + options.tabletMaxWidth);
		...
		//console.log('mobile device ' + isMobileDevice + " .. tablet device " + isTabletDevice);
		...
		if (isMobileDevice || isTabletDevice)
		{
			try
			{
				...
				{
					CSSMenus.insertMobileParent(Name);
					var BtnParent = $("#FM_CSS_" + Name + "_mobile_container");

					var Menu = new XTDMobileMenu(Name);
					// Initialize the menu
					Menu.Init(options);
					Menu.SetParent($("#FM_CSS_"+Name+"_mobile_container").get(0));
	...
	function flexiCSSMenus_getButtonMenu(selectedNode) {
		try {
			...
			//log("##getSelectedLevel##" + selectedNode.parentNode);
			while (selectedNode && (selectedNode[0].className.toLowerCase().indexOf("fm_css_") == -1 || selectedNode[0].className.toLowerCase().indexOf("_container") == -1) ) {
				//log("##selectedNode.parentNode.nodeName.toLowerCase()##" + selectedNode.parentNode.nodeName.toLowerCase());
				selectedNode = selectedNode.parent();
			}
		} catch (e) {
	...
	function XTDMobileMenu(MenuName)
	{
		 ...
		 */
		this.Create = function (ButtonParent)
		{
			CreateMainButton(ButtonParent);

			MainContainer = document.createElement("div");

			MainContainer.setAttribute("id", MenuName+"_jq_menu_back");
			MainContainer.setAttribute("class", "ui-main-container ui-corner-all ui-mobile-viewports");
			...
			MainContainer.style.height = parseInt($(Container).height()) + "px";
			$(MainContainer).hide();
		...
		/*
		 *	SetParent() - Set the menu parent.
		 */
		this.SetParent = function (Value)
		{
			// If the value is undefined then we leave the Parent as it is.
			Parent = (typeof(Value) != "undefined")? Value : Parent;
		}
		...
		function UpdateLinks(Cont)
		{
			...
			// Do some init here.
			$(ObjContainer).children("ul").find("a").click(function (event)
			{
				...
				{
					Self.GoToSubmenu("", this);
					event.preventDefault();
				}
				else
				{
					$(MenuButton).children("span.caption").children("p.xtd_menu_ellipsis").html($(this).children("p").html());
				}
			});
		}
...
(function() {
	...
	function getScriptFolder() {
		...
		for (var i = 0; i < scripts.length; i++) {
			...
			if (src && src.indexOf('flexi_css_menu.js') != -1) {
	...
	if (!window.flexiCssMenus) window.flexiCssMenus = {};
	window.flexiCssMenus.browser = browser;
